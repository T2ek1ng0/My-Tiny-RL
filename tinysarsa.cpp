...
void fun2()
{
    f1.open("onlinedata.txt", ios::out);
    for (ll i = 0; i < 2;++i)
    {
        for (ll j = 0; j < 3;++j) f1 << vec[i][j] <<" ";
        f1 << '\n';
...
int main()
{
    ...
    cout <<vec[1][2]<< '\n';
    ld t=(rand()%100)/100.0;
    if(t>epsilon) action0 = vec[0][state0] > vec[1][state0] ? 0 : 1;
    else action0 = rand() % 2;
    state1 = fun3(state0,action0);
    t = (rand() % 100) / 100.0;
    if(t>epsilon) action1 = vec[0][state1] > vec[1][state1] ? 0 : 1;
    else action1 = rand() % 2;
    vec[action0][state0] += alpha*(reward[state1] + gamma*vec[action1][state1]-vec[action0][state0]);
    state0 = state1;
    action0 = action1;
    for (int i = 1; i < 100; ++i)
    {
